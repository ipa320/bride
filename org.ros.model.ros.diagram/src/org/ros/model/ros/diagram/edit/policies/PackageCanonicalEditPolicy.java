...
public class PackageCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	protected List getSemanticChildrenList() {
		...
		List<RosNodeDescriptor> childDescriptors = RosDiagramUpdater
				.getPackage_1000SemanticChildren(viewObject);
		for (RosNodeDescriptor d : childDescriptors) {
	...
	protected void refreshSemantic() {
		...
		for (Iterator<RosNodeDescriptor> descriptorsIterator = childDescriptors
				.iterator(); descriptorsIterator.hasNext();) {
			RosNodeDescriptor next = descriptorsIterator.next();
			String hint = RosVisualIDRegistry.getType(next.getVisualID());
		...
		for (RosNodeDescriptor next : childDescriptors) {
			String hint = RosVisualIDRegistry.getType(next.getVisualID());
	...
	private Collection<IAdaptable> refreshConnections() {
		...
		Collection<RosLinkDescriptor> linkDescriptors = collectAllLinks(
				getDiagram(), domain2NotationMap);
		Collection existingLinks = new LinkedList(getDiagram().getEdges());
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
				.hasNext();) {
			Edge nextDiagramLink = (Edge) linksIterator.next();
			int diagramLinkVisualID = RosVisualIDRegistry
			...
			for (Iterator<RosLinkDescriptor> linkDescriptorsIterator = linkDescriptors
					.iterator(); linkDescriptorsIterator.hasNext();) {
				RosLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator
						.next();
	 ...
	 * @generated
	 */
	private Collection<RosLinkDescriptor> collectAllLinks(View view,
	...
	private Collection<IAdaptable> createConnections(
			Collection<RosLinkDescriptor> linkDescriptors,
			Map<EObject, View> domain2NotationMap) {
		LinkedList<IAdaptable> adapters = new LinkedList<IAdaptable>();
		for (RosLinkDescriptor nextLinkDescriptor : linkDescriptors) {
			...
			CreateConnectionViewRequest.ConnectionViewDescriptor descriptor = new CreateConnectionViewRequest.ConnectionViewDescriptor(
					...
					RosVisualIDRegistry.getType(nextLinkDescriptor
