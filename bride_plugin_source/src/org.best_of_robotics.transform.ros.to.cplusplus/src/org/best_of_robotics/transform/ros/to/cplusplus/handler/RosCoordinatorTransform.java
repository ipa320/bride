 ...
 *
 */
public class RosCoordinatorTransform extends AbstractHandler {
	
	public RosCoordinatorTransform() {		
	}
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (editor != null) {
            ...
            if (input instanceof IFileEditorInput) {
            	...
            	if(sourcefile.getFileExtension().compareTo("ros_coordinator") != 0 && sourcefile.getFileExtension().compareTo("ros_coordinator_diagram")!=0)
            	{
            		sourcefile = null;
            		MessageDialog
					.openError(
		...
		System.out.println("Taking transformation: " + template_dir + "/python/coordinator/package.egl");
		
		IEglTransformParameter eglTransformParameter = TransformParameterFactory.createEglTransformParameter();
		eglTransformParameter.setTransformName("ROS coordinator to smach implementation");
		eglTransformParameter.setEglTransform("file://" + template_dir + "/python/coordinator/package.egl");
		eglTransformParameter.setPluginID(Activator.PLUGIN_ID);
		eglTransformParameter.setSourceMetaModelURI("http://ros/1.0");
		eglTransformParameter.setSourceModelFilePath(sourcefile.getLocation().removeFileExtension().toOSString() + ".ros_coordinator");
