	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (editor != null) {
			...
			if (input instanceof IFileEditorInput) {
				...
				System.out.println("File from editor: "
						+ sourcefile.getLocation().toOSString());
				if (sourcefile.getFileExtension().compareTo("ros_package") != 0
						&& sourcefile.getFileExtension().compareTo(
								"ros_package_diagram") != 0
						&& sourcefile.getFileExtension().compareTo(
								"ros_package_dsl") != 0) {
					sourcefile = null;
					MessageDialog
		...
		System.out.println("Taking transformation: " + template_dir
				+ "/cpp/package/package.egl");

		IEglTransformParameter eglTransformParameter = TransformParameterFactory
				.createEglTransformParameter();
		eglTransformParameter
				.setTransformName("ROS Pacakge to cpp implementation");
		eglTransformParameter.setEglTransform("file://" + template_dir
				+ "/cpp/package/package.egl");
		eglTransformParameter.setPluginID(Activator.PLUGIN_ID);
		eglTransformParameter.setSourceMetaModelURI("http://ros/1.0");
		if (sourcefile.getFileExtension().compareTo("ros_package_diagram") == 0) {
			eglTransformParameter.setSourceModelFilePath(sourcefile
					.getLocation().removeFileExtension().toOSString()
					+ ".ros_package");
		} else {
			eglTransformParameter.setSourceModelFilePath(sourcefile
		...
		eglTransformParameter.setOutputRoot("file:"
				+ sourcefile.getProject().getLocation().toOSString());
