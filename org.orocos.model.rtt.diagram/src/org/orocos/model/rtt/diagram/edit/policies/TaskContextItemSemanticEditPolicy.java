...
import org.orocos.model.rtt.diagram.edit.commands.InputPortCreateCommand;
import org.orocos.model.rtt.diagram.edit.commands.OutputPortCreateCommand;
...
public class TaskContextItemSemanticEditPolicy extends
	...
	protected Command getCreateCommand(CreateElementRequest req) {
		if (RttElementTypes.OutputPort_3001 == req.getElementType()) {
			return getGEFWrapper(new OutputPortCreateCommand(req));
		}
		if (RttElementTypes.InputPort_3002 == req.getElementType()) {
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (RttVisualIDRegistry.getVisualID(incomingLink) == IActivityTaskContextEditPart.VISUAL_ID) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			case OutputPortEditPart.VISUAL_ID:
				for (Iterator<?> it = node.getSourceEdges().iterator(); it
					...
					if (RttVisualIDRegistry.getVisualID(outgoingLink) == ConnectionPolicyEditPart.VISUAL_ID) {
			...
			case InputPortEditPart.VISUAL_ID:
				for (Iterator<?> it = node.getTargetEdges().iterator(); it
						.hasNext();) {
					Edge incomingLink = (Edge) it.next();
					if (RttVisualIDRegistry.getVisualID(incomingLink) == ConnectionPolicyEditPart.VISUAL_ID) {
						DestroyElementRequest r = new DestroyElementRequest(
	...
	protected Command getStartCreateRelationshipCommand(
		...
		if (RttElementTypes.IActivityTaskContext_4002 == req.getElementType()) {
			return null;
		}
		return null;
	}
	...
	protected Command getCompleteCreateRelationshipCommand(
			CreateRelationshipRequest req) {
		if (RttElementTypes.IActivityTaskContext_4002 == req.getElementType()) {
			return getGEFWrapper(new IActivityTaskContextCreateCommand(req,
	...
	protected Command getReorientReferenceRelationshipCommand(
		...
		case IActivityTaskContextEditPart.VISUAL_ID:
			return getGEFWrapper(new IActivityTaskContextReorientCommand(req));
		}
